...
//	TProof * p = TProof::Open("ccaplmaster.in2p3.fr");
	gSystem->Load("/sps/cms/jfan/CMSSW_4_2_8_patch7/src/Morgan/IpnTreeProducer/src/libToto.so");
...
// INSERTFILES
	...
	TFile* OutputRootFile = new TFile(Form("miniTree_%s_part%i_%i.root", sample.c_str(), ijob, isZgammaMC), "RECREATE");
	TFile* OutputFriendFile = new TFile(Form("miniFriend_%s_part%i_%i.root", sample.c_str(), ijob, isZgammaMC), "RECREATE");
...
//		cout << "loop over nbMuMuGammaAfterID[3]= " << nbMuMuGammaAfterID[3] << " candidates" << endl;
      ...
      TLorentzVector *correctedMuon2 = new TLorentzVector(corrected_Px2, corrected_Py2, corrected_Pz2, corrected_E2);
			...
			double lead_muonPt = (mymuon1->Pt() > mymuon2->Pt() ) ? mymuon1->Pt() : mymuon2->Pt();
			//if( far_muonPt <= 30.0 )
			if( far_muonPt <= 15.0 )     //FAN
			//if( lead_muonPt <= 15.0 )     //FAN
			{
...
// --------------------------------------------------------------------------------------------------------------------// ----- cut on mumugamma.M() (Yang lose window) ----- 
// --------------------------------------------------------------------------------------------------------------------
