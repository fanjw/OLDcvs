...
//	TruncatedPyramid a;
	...
	string line;
	string filename = Form("list_%s", sample_char);
	...
	if (myfile.is_open())
	{
		while( myfile.good() )
		{
			...
			if( iline >= ilineBegin && iline < ilineEnd )
			{
				...
				inputEventTree->Add(Form("%s",line.c_str()));
				inputRunTree->Add(Form("%s",line.c_str()));
			}
		}
		myfile.close();
	} else {
...
*/
  ...
  string ListWantedHLTnames[1];
	...
	TTree* miniTreeSC = new TTree("miniTreeSC","non-associated SC study");     //FAN
...
//	TTree *outputEventTree = inputEventTree->CloneTree(0);
  ...
  // ___________________________________________
	...
	miniTree->Branch("mmg_logs_MZ_Muons_RECO_MC",&mmg_logs_MZ_Muons_RECO_MC,"mmg_logs_MZ_Muons_RECO_MC/F");
 	...
 	miniTreeSC->Branch("SC_rawEt","std::vector<float>",&SC_rawEt);    //FAN
 	miniTreeSC->Branch("pho_Et","std::vector<float>",&pho_Et);    //FAN
 	miniTreeSC->Branch("NnonSC",&NnonSC,"NnonSC/I");    //FAN
...
//cout << "nGenVertices= " << nGenVertices << "\tweight_pileUp= " << weight_pileUp << endl;
		...
		NSC = superClusters->GetEntries();     //FAN
...
//			correctedMuon2->Delete();
		...
		if(verbosity>3) cout << "nCandidate[3]= " << nCandidate[4] << endl;
      ...
      TLorentzVector *correctedMuon2 = new TLorentzVector(corrected_Px2, corrected_Py2, corrected_Pz2, corrected_E2);
			...
			if( min_DeltaR >= 0.8 )
			{
				...
				for(int isc = 0; isc < NSC; isc++){
		                       ...
		                       float rawEt = mySC->rawEnergy()*TMath::Sin(2*TMath::ATan(TMath::Exp(-mySC->crackCorrectionEta())));
                                       if( rawEt > 5.0 && rawEt < 10.0 && !(mySC->isPhoton()) ){
                                             double phiSC = mySC->crackCorrectionPhi();
                                             double etaSC = mySC->crackCorrectionEta();
                                             double phiMuon1 = mymuon1->Phi();
					     double etaMuon1 = mymuon1->Eta();								                                                double phiMuon2 = mymuon2->Phi();											                   double etaMuon2 = mymuon2->Eta();  
  				             double deltaRphomu1 = DeltaR(etaSC, phiSC, etaMuon1, phiMuon1);
					     double deltaRphomu2 = DeltaR(etaSC, phiSC, etaMuon2, phiMuon2);
					     double min_DeltaRSC = min(deltaRphomu1, deltaRphomu2);
                                             if( min_DeltaRSC >= 0.8 ){
					        NnonSC++;          //FAN
					        PSC_rawEt->reserve(NnonSC);         //FAN 
                                                PSC_rawEt->push_back(rawEt);          
                                                cout<<rawEt<<endl;                
			                     }
				       }
				 }
	                         ...
	                         if(NnonSC != 0){
                                    NnonSCpho++;
				    Ppho_Et->reserve(NnonSCpho);
				    Ppho_Et->push_back(Photon_scale[MuMuGammaCandidates[3][i_mmg].first]*(myphoton->Et()));
				 }
      ...
      cout << "TOTALnbPhotonsAfterID["<<i<<"]=\t" << TOTALnbPhotonsAfterID[i] << "\t\t" << "TOTALnbEventsAfterPhotonID["<<i<<"]=\t" << TOTALnbEventsAfterPhotonID[i] << endl;
		...
		for(int i = 0; i < 8 ; i++)
		{
			...
			if(i == 6) cout << endl;
                ...
                cout << "nb of non-associated events = " << totalNeventSC << endl; 
